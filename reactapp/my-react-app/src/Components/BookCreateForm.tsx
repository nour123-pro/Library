...
export default function BookCreateForm({ show, onClose}: { show: boolean, onClose: () => void}) {
    ...
    const[bookimage,setbookimage]=useState<File>();
     ...
     const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const handleSubmit = async (e: React.FormEvent) => {
           ...
           try {
               const response = await axios.post('http://localhost:5000/api/Book/CreateBook', requestdata, {
               headers: {
    ...
    {authors!=null && categories!=null}{
           ...
           <div style={{ width: "50%", height: '60%', alignItems: "center", margin: '0' }} className={`LoginForm ${show ? "show" : ""}`}>
                   <form style={{ width: "100%", height: "100%" }} className="EditProfileForm flex-colomn border-none" onSubmit={handleSubmit}>
                       ...
                       <h2 style={{ alignSelf: 'center', marginTop: '2%' }}>ADD NEW BOOK</h2>
...
>
  ...
  {categories?.map((category: Category) => (
    ...
    onChange={(e) => {
      const selectedAuthor = authors?.find(a => a.authorId === e.target.value);
      setbookauthor(selectedAuthor || null);
    }}
    required
  >
                               <option value="" disabled>Select a Author</option>
                              {authors?.map((author: Author) => (
                                <option value={author.authorId} key={author.authorId} >
                                  {author.authorName}
                                </option>
                              ))}
                           </select>

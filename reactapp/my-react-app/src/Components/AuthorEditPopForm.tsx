...
export default function AuthorEditPopForm({author,show,onClose}:{author:Author,show:boolean,onClose: () => void}){
     const[authorname,setauthorname]=useState<string>(author.authorName);
     const[authorimage,setauthorimage]=useState<File>();
      const [imageFile, setImageFile] = useState<File | null>(null);
        const [imagePreview, setImagePreview] = useState<string>(`data:image;base64,${author.authorImage}`);
      ...
      const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
         ...
         const handleSubmit = async (e: React.FormEvent) => {
            ...
            try {
                const response = await axios.post('http://localhost:5000/api/Authors/EditAuthorInfo', requestdata, {
                    headers: {
    ...
    return(
            <div style={{ width: "50%", height: '60%', alignItems: "center", margin: '0' }} className={`LoginForm ${show ? "show" : ""}`}>
                    <form style={{ width: "100%", height: "100%" }} className="EditProfileForm flex-colomn border-none" onSubmit={handleSubmit}>
                        <button type="button" onClick={onClose} className="text-gray-500 hover:text-red-700 text-8xl ml-auto " >&times;</button>
  ...
  <img
    ...
    src={
        imagePreview ?? (author.authorImage ? `data:image;base64,${author.authorImage}` : '')
      }
